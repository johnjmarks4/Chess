...
describe Bishop do
	...
	describe "#show_moves" do
		...
		it "shows descending-right moves" do
			...
			game.board[0][0] = bishop

			expect(bishop.show_moves(game)).to include [1, 1]
			expect(bishop.show_moves(game)).to include [7, 7]
		end

		it "shows descending-left moves" do
			game.board.each { |rows| rows.map! { |squares| squares = "  " } }
			bishop = Bishop.new(0, 7, "b", "b")
			game.board[0][7] = bishop

			expect(bishop.show_moves(game)).to include [1, 6]
			expect(bishop.show_moves(game)).to include [7, 0]
		end

		it "shows ascending-left moves" do
			game.board.each { |rows| rows.map! { |squares| squares = "  " } }
			bishop = Bishop.new(7, 7, "b", "b")
			game.board[7][7] = bishop

			expect(bishop.show_moves(game)).to include [6, 6]
			expect(bishop.show_moves(game)).to include [0, 0]
		end

		it "shows ascending-right moves" do
			game.board.each { |rows| rows.map! { |squares| squares = "  " } }
			bishop = Bishop.new(7, 0, "b", "b")
			game.board[7][0] = bishop

			expect(bishop.show_moves(game)).to include [6, 1]
			expect(bishop.show_moves(game)).to include [0, 7]
		end
	end
end